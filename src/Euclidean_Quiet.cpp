...
void setup() {
  ...
  for (int a = 0; a < NUM_CHANNELS; a++) {
    generated_rhythms[a] = euclidean_rhythm_gen(channelbeats[a][0], channelbeats[a][1], channelbeats[a][3]);
  }
...
void loop() {
  ...
  */
  ...
  if (changes > 0) {
    generated_rhythms[active_channel] = euclidean_rhythm_gen(active_length, active_density, active_offset);
